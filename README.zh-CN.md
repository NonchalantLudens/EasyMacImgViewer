...
- **文件夹感知导航** — 打开文件时自动扫描同目录图像，按 Finder 自然顺序排列；鼠标（点击左右边缘）、键盘（←/→/空格）或侧边栏切换
- **侧边栏** — 可收回的图像列表（⌘⌥S），缩略图/列表双模式，点击切换
...
- **照片文件夹（iPhone 所有数据）** — 可选模式（View 菜单），将 iPhone「所有数据」传输的每照片文件夹视为一张图像，并提供编辑版优先开关
- **动图** — GIF / 动画 WebP 帧播放
...
| 播放 / 暂停动图 | 工具栏播放按钮 |
| 照片文件夹模式 | 显示菜单 →「将照片文件夹识别为图像（iPhone 所有数据）」 |
